...
export const AppLayout: React.FC<AppLayoutProps> = ({ allowedRoles }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-colors duration-150 flex">
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 lg:pl-64 bg-slate-50 dark:bg-slate-950">
        <Navbar onToggleSidebar={() => setSidebarOpen((prev) => !prev)} />
        <main className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl w-full mx-auto bg-slate-50 dark:bg-slate-950">
